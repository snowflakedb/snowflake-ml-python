...
bazel "--output_user_root=C:\broot" "build" --config=build "--repository_cache=" "--nobuild_python_zip" "--enable_runfiles" --action_env="USERPROFILE=%USERPROFILE%" --host_action_env="USERPROFILE=%USERPROFILE%" --config="py%PY_VER%" "//:wheel" || exit /b %errorlevel%
SET BAZEL_BIN_PATH=
