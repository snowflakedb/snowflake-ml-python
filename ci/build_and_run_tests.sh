...
for i in "${!groups[@]}"; do
    ...
    if [ "${ENV}" = "pip" ]; then
        ...
        python -m pip list
        python -m pip install "snowflake_ml_python-${VERSION}-py3-none-any.whl[all]" -r "${WORKSPACE}/${SNOWML_DIR}/bazel/environments/requirements_${group}.txt" --no-cache-dir --force-reinstall
        if [ "${WITH_SNOWPARK}" = true ]; then
...
for exit_code in "${group_exit_codes[@]}"; do
    if [[ (${MODE} = "merge_gate" || ${MODE} = "quarantined") && ${exit_code} -eq 5 ]]; then
        continue
    fi
