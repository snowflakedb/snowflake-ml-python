...
class SqlIdentifier(str):
    ...
    def __new__(cls, name: str, quotes_to_preserve_case: bool = True) -> "SqlIdentifier":
        ...
        Args:
            name: A string name.
            quotes_to_preserve_case: If true then double quotes are needed to preserve case. For example:
                    abc     -> ABC
                    "abc"   -> abc
                When it's false, case are preserved automatically. For example:
                    abc     -> abc
                    ABC     -> ABC
