...
class FormattingTest(TestCase):
    def test_format_value_for_select_str(self) -> None:
        ...
        self.assertEqual(formatting.format_value_for_select("string"), "'string'")

    def test_format_value_for_select_sql(self) -> None:
        """Test that SQL strings will not be quoted."""
        self.assertEqual(formatting.format_value_for_select(formatting.SqlStr("CURRENT_TIME()")), "CURRENT_TIME()")
    ...
    def test_format_value_for_select_datetime(self) -> None:
        ...
        self.assertEqual(formatting.format_value_for_select(dt), "TO_TIMESTAMP('2011-11-04T00:05:23+04:00')")

    def test_format_value_for_select_dict(self) -> None:
        """Test dictionary formatting for the value in a select clause."""
        self.assertEqual(
            formatting.format_value_for_select({"key": "value_str"}), "OBJECT_CONSTRUCT('key','value_str')"
        )
        self.assertEqual(formatting.format_value_for_select({"key": 2.71828}), "OBJECT_CONSTRUCT('key',2.71828)")
        self.assertEqual(
            formatting.format_value_for_select({"key": {"nested_key": "value_str"}}),
    ...
    def test_format_value_for_select_dict_sorting(self) -> None:
        ...
        self.assertEqual(
            formatting.format_value_for_select({"A": "a", "B": "b", "C": "c"}),
            "OBJECT_CONSTRUCT('A','a','B','b','C','c')",
        )
    ...
    def test_format_value_for_select_numpy_array(self) -> None:
        ...
        self.assertEqual(formatting.format_value_for_select(np.array([0, 1])), "ARRAY_CONSTRUCT(0,1)")
        self.assertEqual(
            formatting.format_value_for_select(np.array([[0, 1], [2, 3]])),
    ...
    def test_format_value_for_select_other(self) -> None:
        ...
        self.assertEqual(formatting.format_value_for_select(23), "23")

    def test_format_value_for_select_null(self) -> None:
        """Test that None/null is correctly handled when formatting values."""
        self.assertEqual(formatting.format_value_for_select(None), "null")

    def test_unwrap(self) -> None:
        """Test string unwrapping."""
        self.assertEqual(formatting.unwrap("  Test\t \n String\r\n"), "Test String")
        self.assertEqual(formatting.unwrap("nothing to do"), "nothing to do")

    def test_unwrap_keep_newlines(self) -> None:
        """Unwrap a string but keep newlines."""
        self.assertEqual(formatting.unwrap("  line1  \n  \t   line2 ", keep_newlines=True), "line1\nline2")
        self.assertEqual(
            formatting.unwrap(
                """Expected
                   Actual""",
