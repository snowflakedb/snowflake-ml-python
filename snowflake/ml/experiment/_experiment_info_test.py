...
from snowflake.ml.experiment._experiment_info import (
    ExperimentInfo,
    ExperimentInfoPatcher,
)
...
class ExperimentInfoPatcherTest(absltest.TestCase):
    ...
    def setUp(self) -> None:
        ...
        self.experiment_info = ExperimentInfo(fully_qualified_name="test.experiment.name", run_name="test_run")
        self.original_log_model = ModelManager.log_model

    def tearDown(self) -> None:
        """Clean up after each test."""
        # Clear the stack
        ExperimentInfoPatcher._experiment_info_stack.clear()
    ...
    def test_context_manager_basic_usage(self) -> None:
        ...
        patcher = ExperimentInfoPatcher(self.experiment_info)
        ...
        with patcher as context_patcher:
            ...
            self.assertEqual(len(ExperimentInfoPatcher._experiment_info_stack), 1)
            self.assertEqual(ExperimentInfoPatcher._experiment_info_stack[0], self.experiment_info)
        ...
        self.assertEqual(len(ExperimentInfoPatcher._experiment_info_stack), 0)
    ...
    def test_patching_only_happens_on_first_context(self) -> None:
        ...
        experiment_info_1 = ExperimentInfo(fully_qualified_name="experiment.1", run_name="run_1")
        experiment_info_2 = ExperimentInfo(fully_qualified_name="experiment.2", run_name="run_2")

        with ExperimentInfoPatcher(experiment_info_1):
            patched_method = ModelManager.log_model

            # Verify method was patched
            self.assertIsNot(patched_method, self.original_log_model)

            with ExperimentInfoPatcher(experiment_info_2):
    ...
    def test_preserves_original_method_signature_and_return_value(self) -> None:
        ...
        ExperimentInfoPatcher._original_log_model = mock_original

        mock_manager = mock.MagicMock(spec=ModelManager)
        mock_event_handler = mock.MagicMock()

        with ExperimentInfoPatcher(self.experiment_info):
