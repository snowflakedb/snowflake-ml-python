...
def _get_fully_qualified_table_name(database_name: str, schema_name: str, table_name: str) -> str:
    return f"{_get_fully_qualified_schema_name(database_name, schema_name)}.{table_name}"
...
def _insert_table_entry(session: snowpark.Session, table: str, columns: Dict[str, Any]) -> List[snowpark.Row]:
...
def _validate_table_exist(session: snowpark.Session, table: str, qualified_schema_name: str) -> bool:
