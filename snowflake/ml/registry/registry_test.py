...
class RegistryTest(absltest.TestCase):
    ...
    def test_get_model_1(self) -> None:
        ...
        with mock.patch.object(self.m_r._model_ops, "validate_existence", return_value=True) as mock_validate_existence:
            ...
            self.assertDictEqual(m.__dict__, m_model.__dict__)
    ...
    def test_list_models(self) -> None:
        m_model_1 = model_impl.Model(
            self.m_r._model_ops,
            model_name=sql_identifier.SqlIdentifier("MODEL"),
        )
        m_model_2 = model_impl.Model(
            self.m_r._model_ops,
        ...
        with mock.patch.object(
            ...
            self.assertDictEqual(m_list[0].__dict__, m_model_1.__dict__)
            self.assertDictEqual(m_list[1].__dict__, m_model_2.__dict__)
            mock_list_models_or_versions.assert_called_once_with(
                statement_params=mock.ANY,
            )

    def test_log_model_bad_case_1(self) -> None:
        m_model = mock.MagicMock()
        with self.assertRaisesRegex(ValueError, "is currently not allowed in the version name"):
            self.m_r.log_model(
                model=m_model,
                model_name="MODEL",
                version_name="v.1",
