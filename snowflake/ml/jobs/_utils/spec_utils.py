...
def _get_node_resources(session: snowpark.Session, compute_pool: str) -> types.ComputeResources:
    ...
    (row,) = session.sql("select current_region()").collect()
    region: str = row[0]
    region_group, region_name = f".{region}".split(".")[-2:]  # Prepend a period so we always get at least 2 splits
            ...
            "containers": [
                {
                    ...
                    "args": [
                        os.path.join(stage_mount, str(v)) if isinstance(v, Path) else v for v in payload.entrypoint
                    ]
                    + (args or []),
                    "env": {
                        constants.PAYLOAD_DIR_ENV_VAR: stage_mount,
                    },
