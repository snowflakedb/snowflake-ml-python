...
class RollupSqlGenerator:
    ...
    def generate(self) -> str:
        ...
        for spec in self._aggregation_specs:
            for col_expr in self._get_rollup_aggs(spec):
        ...
        # Find the first new key that is NOT in parent keys for NULL filtering
        # If all keys are shared, use the first one
        not_null_key = None
        for key in self._new_join_keys:
        ...
        if not_null_key in self._parent_join_keys:
            not_null_expr = f"t.{not_null_key} IS NOT NULL"
        else:
            not_null_expr = f"m.{not_null_key} IS NOT NULL"
...
"""
    ...
    def _get_rollup_aggs(self, spec: AggregationSpec) -> list[str]:
        ...
        if spec.function == AggregationType.COUNT:
            ...
            results.append(f"SUM(t.{col}) AS {col}")
            ...
            sum_col = spec.get_tile_column_name("SUM")
            count_col = spec.get_tile_column_name("COUNT")
            results.append(f"SUM(t.{sum_col}) AS {sum_col}")
            results.append(f"SUM(t.{count_col}) AS {count_col}")

        elif spec.function == AggregationType.MIN:
            col = spec.get_tile_column_name("MIN")
            results.append(f"MIN(t.{col}) AS {col}")

        elif spec.function == AggregationType.MAX:
            col = spec.get_tile_column_name("MAX")
            results.append(f"MAX(t.{col}) AS {col}")
            ...
            results.append(f"SUM(t.{sum_col}) AS {sum_col}")
            results.append(f"SUM(t.{count_col}) AS {count_col}")
            results.append(f"SUM(t.{sum_sq_col}) AS {sum_sq_col}")

        elif spec.function == AggregationType.APPROX_COUNT_DISTINCT:
            # HLL state is stored as an exported object via HLL_EXPORT(HLL_ACCUMULATE(...))
            # We need to HLL_IMPORT (convert back to binary), combine, then HLL_EXPORT to maintain the format
            col = spec.get_tile_column_name("HLL")
            results.append(f"HLL_EXPORT(HLL_COMBINE(HLL_IMPORT(t.{col}))) AS {col}")

        elif spec.function == AggregationType.APPROX_PERCENTILE:
            # T-Digest state needs APPROX_PERCENTILE_COMBINE
            col = spec.get_tile_column_name("TDIGEST")
            results.append(f"APPROX_PERCENTILE_COMBINE(t.{col}) AS {col}")

        elif spec.function in (AggregationType.LAST_N, AggregationType.LAST_DISTINCT_N):
            # List aggregations: use ARRAY_UNION_AGG to merge arrays from child entities
            col = spec.get_tile_column_name("LAST")
            results.append(f"ARRAY_UNION_AGG(t.{col}) AS {col}")

        elif spec.function in (AggregationType.FIRST_N, AggregationType.FIRST_DISTINCT_N):
            col = spec.get_tile_column_name("FIRST")
            results.append(f"ARRAY_UNION_AGG(t.{col}) AS {col}")
