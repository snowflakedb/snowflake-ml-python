...
class FeatureStore:
    ...
    def register_feature_view(
        ...
        if feature_view.status != FeatureViewStatus.DRAFT:
            warnings.warn(
                ...
                + "likely it is already registered. Skip registration.",
                stacklevel=2,
                category=UserWarning,
            )
            return feature_view
        ...
        if not overwrite:
            ...
            if len(dynamic_table_results) > 0 or len(view_results) > 0:
                warnings.warn(
                    f"FeatureView {feature_view.name}/{feature_view.version} already exists. Skip registration."
    ...
    def _check_internal_objects_exist_or_throw(self) -> None:
        ...
        if len(schema_result) == 0:
            raise snowml_exceptions.SnowflakeMLException(
                error_code=error_codes.NOT_FOUND,
                original_exception=ValueError(f"Feature store schema {self._config.schema} does not exist."),
            )
        for tag_name in to_sql_identifiers(
            ...
            if len(tag_result) == 0:
                raise snowml_exceptions.SnowflakeMLException(
                    error_code=error_codes.NOT_FOUND,
                    original_exception=ValueError(f"Feature store internal tag {tag_name} does not exist."),
                )
