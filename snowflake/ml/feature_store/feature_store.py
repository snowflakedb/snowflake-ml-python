...
class FeatureStore:
        ...
        try:
            ...
            for obj_type in object_types:
                ...
                for row in all_object_rows:
                    obj_name = self._get_fully_qualified_name(SqlIdentifier(row["name"], False))
            ...
            all_tags = [
                ...
                FEATURE_VIEW_TS_COL_TAG,
                FEATURE_STORE_OBJECT_TAG,
            ] + [SqlIdentifier(row["name"], False) for row in entity_tags]
    ...
    def _composed_union_window_join_query(
        ...
        prefix_f_only_cols = to_sql_identifiers([f"{temp_prefix}{name.resolved()}" for name in f_only_cols], False)
        last_select = f"""
            SELECT
                {join_keys_str},
                {s_ts_col},
    ...
    def _find_feature_views(
        ...
        all_fv_names = [SqlIdentifier(r["name"], False) for r in self._get_backend_representations(None)]
        ...
        outputs = []
        for r in results:
            if entity_name == SqlIdentifier(r["TAG_VALUE"], False):
                fv_name, version = to_sql_identifiers(r["OBJECT_NAME"].split(FEATURE_VIEW_NAME_DELIMITER), False)
                if feature_view_name is not None:
    ...
    def _compose_feature_view(self, row: Row) -> FeatureView:
        name, version = to_sql_identifiers(row["name"].split(FEATURE_VIEW_NAME_DELIMITER), False)
        ...
        if m is not None:
            ...
            fv = FeatureView._construct_feature_view(
                ...
                feature_descs=self._fetch_column_descs("DYNAMIC TABLE", SqlIdentifier(row["name"], False)),
        ...
        if m is not None:
            ...
            fv = FeatureView._construct_feature_view(
                ...
                feature_descs=self._fetch_column_descs("VIEW", SqlIdentifier(row["name"], False)),
                refresh_freq=None,
                database=self._config.database.identifier(),
                schema=self._config.schema.identifier(),
                warehouse=None,
    ...
    def _fetch_column_descs(self, obj_type: str, obj_name: SqlIdentifier) -> Dict[str, str]:
        ...
        for r in res:
            if r["comment"] is not None:
                descs[SqlIdentifier(r["name"], False).identifier()] = r["comment"]
        return descs
            ...
            if object_type not in tag_free_object_types and len(all_rows) > 0:
                ...
                queries = [
                    f"""
                        ...
                        FROM TABLE(
                            {self._config.database}.INFORMATION_SCHEMA.TAG_REFERENCES(
                                '{self._get_fully_qualified_name(SqlIdentifier(row['name'], False))}',
                                '{obj_domain}'
                            )
                        )
