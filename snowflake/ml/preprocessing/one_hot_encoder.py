...
from snowflake.ml._internal import type_utils as snowml_type_utils
from snowflake.ml.framework import _utils, base
from snowflake.ml.utils import telemetry
from snowflake.snowpark import functions, types
from snowflake.snowpark._internal import (
    type_utils as snowpark_type_utils,
    utils as snowpark_utils,
)
...
class OneHotEncoder(base.BaseEstimator, base.BaseTransformer):
    ...
    def __init__(
        self,
        *,
        categories: Union[str, Dict[str, snowml_type_utils.LiteralNDArrayType]] = "auto",
        ...
        # Fit state
        self.categories_: Dict[str, snowml_type_utils.LiteralNDArrayType] = {}
        self._categories_list: List[snowml_type_utils.LiteralNDArrayType] = []
    ...
    @property
    def infrequent_categories_(self) -> List[Optional[snowml_type_utils.LiteralNDArrayType]]:
    ...
    def _get_category_count_state_df(self, dataset: snowpark.DataFrame) -> snowpark.DataFrame:
        ...
        for input_col in self.input_cols:
            ...
            condition = typing.cast(
                snowpark_type_utils.ColumnOrSqlExpr,
                functions.col(input_col).is_null() | (functions.col(input_col).cast(types.StringType()).equal_nan()),
            )
            # count null and nan values
            expr1 = typing.cast(
                snowpark_type_utils.ColumnOrLiteral,
                functions.sum(
                    typing.cast(
                        snowpark_type_utils.ColumnOrName,
                        functions.iff(
                            typing.cast(
                                snowpark_type_utils.ColumnOrSqlExpr,
                                functions.col(input_col).is_null()
                                | (functions.col(input_col).cast(types.StringType()).equal_nan()),
                            ),
                            typing.cast(snowpark_type_utils.ColumnOrLiteral, 1),
                            typing.cast(snowpark_type_utils.ColumnOrLiteral, 0),
                        ),
                    )
                ).over(snowpark.Window.partition_by(typing.cast(snowpark_type_utils.ColumnOrName, input_col))),
            )
            # count values that are not null or nan
            expr2 = typing.cast(
                snowpark_type_utils.ColumnOrLiteral,
                functions.count(typing.cast(snowpark_type_utils.ColumnOrName, input_col)).over(
                    snowpark.Window.partition_by(typing.cast(snowpark_type_utils.ColumnOrName, input_col))
                ),
            )
            state_columns = [
                functions.lit(typing.cast(snowpark_type_utils.LiteralType, input_col)).alias(_COLUMN_NAME),
                functions.col(input_col).cast(types.StringType()).alias(_CATEGORY),
                functions.iff(condition, expr1, expr2).alias(_COUNT),
            ]
    ...
    def _assign_categories(self, state_object_pandas: pd.DataFrame) -> None:
        ...
        if isinstance(self.categories, str):
            ...
            categories: Dict[str, snowml_type_utils.LiteralNDArrayType] = categories_pandas.set_index(
                _COLUMN_NAME
            ).to_dict()[categories_col]
    ...
    def _handle_unknown_in_transform(self, transformed_dataset: snowpark.DataFrame) -> snowpark.DataFrame:
        ...
        if self.handle_unknown == "error":
            ...
            for idx, input_col in enumerate(self.input_cols):
                ...
                unknown_columns = [
                    functions.lit(typing.cast(snowpark_type_utils.LiteralType, input_col)),
        ...
        if self.handle_unknown == "ignore" and not self.sparse:
            ...
            transformed_dataset = transformed_dataset.na.fill(
                typing.cast(snowpark_type_utils.LiteralType, 0), all_output_cat_cols
            )
