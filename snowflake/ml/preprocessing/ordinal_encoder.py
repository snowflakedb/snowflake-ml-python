...
from snowflake.snowpark._internal import type_utils as snowpark_type_utils
...
class OrdinalEncoder(base.BaseEstimator, base.BaseTransformer):
    ...
    def _get_category_index_state_df(self, dataset: snowpark.DataFrame) -> snowpark.DataFrame:
        ...
        for input_col in self.input_cols:
            ...
            encoded_value_columns = [
                ...
                (
                    functions.dense_rank().over(
                        snowpark.Window.order_by(typing.cast(snowpark_type_utils.ColumnOrName, input_col))
                    )
            ...
            encoded_missing_value_columns = [
                functions.lit(typing.cast(snowpark_type_utils.LiteralType, input_col)).alias(_COLUMN_NAME),
                functions.col(input_col).alias(_CATEGORY),
                functions.lit(typing.cast(snowpark_type_utils.LiteralType, self.encoded_missing_value)).alias(
                    _INDEX
    ...
    def _transform_snowpark(self, dataset: snowpark.DataFrame) -> snowpark.DataFrame:
        ...
        null_category_state_df = state_df.filter(functions.col(_CATEGORY).is_null()).with_column(
            _INDEX, functions.lit(typing.cast(snowpark_type_utils.LiteralType, self.encoded_missing_value))
        )
        ...
        if self.handle_unknown == "use_encoded_value":
            ...
            if not (self.unknown_value is None or sklearn_utils.is_scalar_nan(self.unknown_value)):
                transformed_dataset = transformed_dataset.na.fill(
                    typing.cast(snowpark_type_utils.LiteralType, self.unknown_value),
                    self.output_cols,
                )
