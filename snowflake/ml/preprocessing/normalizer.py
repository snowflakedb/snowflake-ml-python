...
class Normalizer(base.BaseEstimator, base.BaseTransformer):
    ...
    def _transform_snowpark(self, dataset: snowpark.DataFrame) -> snowpark.DataFrame:
        ...
        if self.norm == "l1":
            norm = functions.lit(typing.cast(type_utils.LiteralType, "0"))
            for input_col in self.input_cols:
                norm += typing.cast(type_utils.ColumnOrLiteral, functions.abs(dataset[input_col]))

        elif self.norm == "l2":
            norm = functions.lit(typing.cast(type_utils.LiteralType, "0"))
            for input_col in self.input_cols:
                norm += typing.cast(type_utils.ColumnOrLiteral, dataset[input_col] * dataset[input_col])
            norm = functions.sqrt(typing.cast(type_utils.ColumnOrName, norm))

        elif self.norm == "max":
            norm = functions.greatest(
                *[
                    typing.cast(type_utils.ColumnOrName, functions.abs(dataset[input_col]))
                    for input_col in self.input_cols
                ]
            )
        ...
        for input_col in self.input_cols:
            ...
            output_column = functions.div0(
                ...
                typing.cast(type_utils.ColumnOrName, norm),
            )
