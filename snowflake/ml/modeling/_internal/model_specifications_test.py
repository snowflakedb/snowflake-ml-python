...
for each_array in each_cv_result:
...
class SnowparkHandlersUnitTest(parameterized.TestCase):
    def setUp(self) -> None:
        ...
        zipped = sorted(zip([5, 4, 2, 0, 1, 3], combine_hex_cv_result), key=lambda x: x[0])
    ...
    def test_cv_result(self) -> None:
        multimetric, cv_results_, best_param_index, scorers = construct_cv_results(
            self.RAW_DATA_SP, IDX_LENGTH, PARAM_LENGTH
        )
        self.assertEqual(multimetric, False)
        self._compare_cv_results(cv_results_, CV_RESULT_)
    ...
    def test_cv_result_incorrect_param_length(self) -> None:
        with self.assertRaises(ValueError):
            construct_cv_results(self.RAW_DATA_SP, IDX_LENGTH, 1)

    def test_cv_result_nan(self) -> None:
        # corner cases with nan values
        with self.assertRaises(ValueError):
            construct_cv_results(self.RAW_DATA_SP, 0, PARAM_LENGTH)
        # empty list
        with self.assertRaises(ValueError):
            construct_cv_results([], IDX_LENGTH, PARAM_LENGTH)
