...
# Initialized to None so the inference function can detect whether load_model() has run yet.
runner = None
...
def infer(df: pd.DataFrame):
    ...
    # Single, non-OBJECT output: return the value directly so the function's native return type is preserved.
    return predictions_df.replace({pd.NA: None, np.nan: None}).iloc[:, 0]
