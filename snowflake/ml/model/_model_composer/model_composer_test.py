...
class ModelInterfaceTest(parameterized.TestCase):
    ...
    def test_save_interface(self, params: dict[str, Any]) -> None:
        ...
        with open(os.path.join(m.packager_workspace_path, "model.yaml"), "w", encoding="utf-8") as f:
            f.write("")
        m.packager = mock_pk
        with mock.patch.object(m.packager, "save", return_value=mock_pk.meta) as mock_save, mock.patch.object(
            m.manifest, "save"
        ) as mock_manifest_save, mock.patch.object(
    ...
    def test_save_enable_explainability(self, disable_explainability: bool, **kwargs: Any) -> None:
        ...
        with mock.patch.object(m.packager, "save", return_value=mock_pk.meta) as mock_save, mock.patch.object(
            m.manifest, "save"
        ), mock.patch.object(file_utils, "upload_directory_to_stage", return_value=None), mock.patch.object(
            env_utils,
            "get_matched_package_versions_in_information_schema",
            return_value={env_utils.SNOWPARK_ML_PKG_NAME: []},
        ):
            ...
            if (
                ...
                self.assertEqual(called_kwargs["options"], called_kwargs["options"] | {"enable_explainability": False})
            else:
        ...
        if disable_explainability:
            ...
            ), mock.patch.object(m.packager, "save", return_value=mock_pk.meta), mock.patch.object(
                m.manifest, "save"
            ), mock.patch.object(
                file_utils, "upload_directory_to_stage", return_value=None
            ), mock.patch.object(
                file_utils, "copytree", return_value="/model"
            ), mock.patch.object(
                env_utils,
                "get_matched_package_versions_in_information_schema",
                return_value={env_utils.SNOWPARK_ML_PKG_NAME: []},
    ...
    def test_save_information_schema_check(self, target_platforms: list[model_types.TargetPlatform]) -> None:
        m_session = mock_session.MockSession(conn=None, test_case=self)
        c_session = cast(Session, m_session)
        ...
        with mock.patch.object(m.packager, "save", return_value=mock_pk.meta) as mock_save, mock.patch.object(
            ...
            "get_matched_package_versions_in_information_schema",
            return_value={env_utils.SNOWPARK_ML_PKG_NAME: []},
        ) as mock_get_matched_package_versions_in_information_schema:
            m.save(
                name="model1",
                model=linear_model.LinearRegression(),
            ...
            if target_platforms == [model_types.TargetPlatform.SNOWPARK_CONTAINER_SERVICES]:
                mock_get_matched_package_versions_in_information_schema.assert_not_called()
            else:
                mock_get_matched_package_versions_in_information_schema.assert_called_once()
