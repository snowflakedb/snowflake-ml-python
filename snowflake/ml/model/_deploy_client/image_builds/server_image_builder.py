    ...
    def _construct_and_upload_docker_entrypoint_script(
        ...
        with open(kaniko_shell_script_template, encoding="utf-8") as template_file, open(
            ...
            params = {
                ...
                "tar_from": "/" + uri.remove_prefix(context_tarball_stage_location, "@"),  # skips the @ prefix
                # Remove @ in the beginning, append "/" to denote root directory.
                "tar_to": "/" + uri.remove_prefix(self.artifact_stage_location, "@"),
                "context_dir": f"dir:///{uri.remove_prefix(self.artifact_stage_location, '@')}/{constants.CONTEXT}",
                "image_repo": self.image_repo,
            ...
            content = Template(template_file.read()).substitute(
                {
                    ...
                    "stage": uri.remove_prefix(self.artifact_stage_location, "@").split("/")[
                        0
                    ],  # Reserve only the stage name
                    # Remove @ in the beginning, append "/" to denote root directory.
                    "script_path": "/" + uri.remove_prefix(kaniko_shell_script_stage_location, "@"),
                }
            )
