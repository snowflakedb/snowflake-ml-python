...
class HuggingFacePipelineHandlerTest(absltest.TestCase):
    ...
    def _basic_test_case(
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            assert isinstance(m, transformers.Pipeline)
            self._check_loaded_pipeline_object(model, m)

            check_pipeline_fn(model, m)

            m_UDF, meta = model_api._load(local_dir_path=os.path.join(tmpdir, "model1"), as_custom_model=True)
            assert callable(m_UDF)
            res = m_UDF(udf_test_input.copy(deep=True))
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            self._check_loaded_pipeline_wrapper_object(wrapper_model, m)
            self.assertEqual(s, meta.signatures)
            ...
            if check_gpu:
                m, meta = model_api._load(
                    local_dir_path=os.path.join(tmpdir, "model1_no_sig"),
                    options={"use_gpu": True},
                )
                assert isinstance(m, huggingface_pipeline.HuggingFacePipelineModel)
                self._check_loaded_pipeline_wrapper_object(wrapper_model, m, use_gpu=True)
                self.assertEqual(s, meta.signatures)
