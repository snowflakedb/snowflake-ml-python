...
class TensorflowHandlerTest(parameterized.TestCase):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            with self.assertRaises(ValueError):
                model_packager.ModelPackager(os.path.join(tmpdir, "model1")).save(
                    ...
                    options=model_types.TensorflowSaveOptions(),
                )
            ...
            x_df = model_signature_utils.rename_pandas_df(
                tensorflow_handler.TensorflowTensorHandler.convert_to_df(data=x, ensure_serializable=False),
                s["__call__"].inputs,
            )
    ...
    def test_tensorflow_multiple_inputs(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            simple_module = SimpleModule(name="simple")
            x = tf.constant([[5.0], [10.0]])
            y_pred = simple_module(x)
            ...
            with self.assertRaises(NotImplementedError):
                model_packager.ModelPackager(os.path.join(tmpdir, "model1")).save(
                    ...
                    signatures=s,
                    metadata={"author": "halu", "version": "1"},
                    options={"enable_explainability": True, "multiple_inputs": True},
                )
