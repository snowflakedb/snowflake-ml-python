...
def validate_pkg_versions_supported_in_snowflake_conda_channel(pkg_versions: List[str], session: Session) -> None:
    for pkg_version in pkg_versions:
        if not _validate_pkg_version_supported_in_snowflake_conda_channel(pkg_version=pkg_version, session=session):
            raise RuntimeError(f"Package {pkg_version} is not supported in snowflake conda channel.")


def _validate_pkg_version_supported_in_snowflake_conda_channel(pkg_version: str, session: Session) -> bool:
    if pkg_version not in cache:
        ...
        result = session.sql(sql).collect(
            statement_params=telemetry.get_function_usage_statement_params(project=_PROJECT, subproject=_SUBPROJECT)
        )
        try:
