...
from snowflake.ml.beta.observability import (
    model_monitor,
    model_monitor_config,
    model_monitor_registry,
    monitor_sql_client,
)
...
class ModelMonitorRegistryIntegrationTest(parameterized.TestCase):
    def _create_test_table(self, fully_qualified_table_name: str):
        self._session.sql(
            f"""CREATE OR REPLACE TABLE {fully_qualified_table_name}
            (label FLOAT, prediction FLOAT, F1 FLOAT, id STRING, timestamp TIMESTAMP)"""
        ).collect()
    ...
    def setUp(self) -> None:
        ...
        model_monitor_registry.ModelMonitorRegistry.setup(
            session=self._session, database_name=self._db_name, schema_name=self._schema_name
        )
    ...
    def _add_sample_model_version_and_monitor(
        self,
        monitor_registry: model_monitor_registry.ModelMonitorRegistry,
        source_table: str,
        model_name: str,
        version_name: str,
        monitor_name: str,
    ...
    def test_add_model_monitor(self) -> None:
        # Create an instance of the ModelMonitorRegistry class
        _monitor_registry = model_monitor_registry.ModelMonitorRegistry(
            session=self._session, database_name=self._db_name, schema_name=self._schema_name
        )
        ...
        for col in table_columns:
            self.assertTrue(col["name"].upper() in ["PREDICTION", "LABEL", "F1", "ID", "TIMESTAMP"])

        df = self._session.create_dataframe(
            [
                (1.0, 1.0, 1.0, "1", "2021-01-01 00:00:00"),
                (2.0, 2.0, 2.0, "2", "2021-01-01 00:00:00"),
            ],
            ["LABEL", "PREDICTION", "F1", "ID", "TIMESTAMP"],
        )
        ...
        pandas_df = pd.DataFrame(
            {
                ...
                "F1": [1.0, 2.0, 3.0],
                "ID": ["1", "2", "3"],
                "TIMESTAMP": ["2021-01-01 00:00:00", "2021-01-01 00:00:00", "2021-01-01 00:00:00"],
            }
        )
        ...
        df = self._session.create_dataframe(
            [
                (1.0, "bad", 1.0, "1", "2021-01-01 00:00:00"),
                (2.0, "very_bad", 2.0, "2", "2021-01-01 00:00:00"),
            ],
            ["LABEL", "PREDICTION", "F1", "ID", "TIMESTAMP"],
        )
