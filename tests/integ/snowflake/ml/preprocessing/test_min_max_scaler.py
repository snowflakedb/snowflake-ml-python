...
class MinMaxScalerTest(TestCase):
    ...
    def test_fit(self) -> None:
        ...
        for _df in [df_pandas, df]:
            ...
            assert np.allclose(actual_min, scaler_sklearn.min_)
            assert np.allclose(actual_scale, scaler_sklearn.scale_)
            assert np.allclose(actual_data_min, scaler_sklearn.data_min_)
            assert np.allclose(actual_data_max, scaler_sklearn.data_max_)
            assert np.allclose(actual_data_range, scaler_sklearn.data_range_)
