...
class GridSearchPipelineTest(TestCase):
    ...
    def test_fit_and_compare_results(self) -> None:
        ...
        pipeline.set_params(**gs.to_sklearn().best_params_)
        assert pipeline.to_sklearn().steps[-1][1].penalty == "l2"
        pipeline.set_params(**{"CLF__penalty": "l1"})
        assert pipeline.to_sklearn().steps[-1][1].penalty == "l1"
