...
from snowflake.ml.ensemble import VotingRegressor
from snowflake.ml.linear_model import LinearRegression, SGDRegressor
...
class VotingRegressorTest(TestCase):
    ...
    def test_fit_and_compare_results(self) -> None:
        ...
        assert np.allclose(actual_arr.flatten(), sklearn_numpy_arr.flatten(), rtol=1.0e-1, atol=1.0e-2)
