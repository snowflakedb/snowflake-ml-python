...
class JobManagerTest(parameterized.TestCase):
    ...
    def test_async_job_parameter(self) -> None:
        try:
            ...
            with self.assertRaisesRegex(RuntimeError, "ENABLE_SNOWSERVICES_ASYNC_JOBS"):
                jm._submit_job(lambda: print("hello world"), self.compute_pool, stage_name="payload_stage")

            self.session.sql("ALTER SESSION SET ENABLE_SNOWSERVICES_ASYNC_JOBS = TRUE").collect()
            job = jm._submit_job(lambda: print("hello world"), self.compute_pool, stage_name="payload_stage")
    ...
    def test_list_jobs(self) -> None:
        ...
        try:
            ...
            self.assertEmpty(jobs.list_jobs().collect())

            # Submit a job
            job = jm._submit_job(lambda: print("hello world"), self.compute_pool, stage_name="payload_stage")

            # Validate list jobs output
            jobs_df = jobs.list_jobs()
            ...
            loaded_job = jobs.get_job(job.id)
            loaded_job.wait()
            self.assertEqual(1, jobs.list_jobs().count())

            # TODO: Test different scopes
            scopes = [
                "account",
                "database",
                f"database {self.db}",
            ...
            for scope in scopes:
                with self.subTest(f"scope={scope}"):
                    self.assertGreater(jobs.list_jobs(scope=scope).count(), 0)

            # Submit a second job to test different limits
            job2 = jm._submit_job(lambda: print("hello world"), self.compute_pool, stage_name="payload_stage")
            limits = [1, 2, 5, 10]
            for limit in limits:
                with self.subTest(f"limit={limit}"):
                    self.assertBetween(jobs.list_jobs(limit=limit, scope="schema").count(), min(limit, 2), limit)
            self.assertEqual(2, jobs.list_jobs(limit=0, scope="schema").count())
            self.assertEqual(2, jobs.list_jobs(limit=-1, scope="schema").count())
            self.assertEqual(2, jobs.list_jobs(limit=-10, scope="schema").count())

            # Delete the job
            jobs.delete_job(job.id)
            jobs.delete_job(job2.id)
            self.assertEqual(0, jobs.list_jobs().count())
        finally:
    ...
    def test_job_wait(self) -> None:
        ...
        try:
            ...
            constants.JOB_POLL_MAX_DELAY_SECONDS = 0.1

            # Create a dummy job
            job = jobs.MLJob("dummy_job_id", self.session)
    ...
    def test_job_execution(self) -> None:
        ...
        job = jobs.submit_file(
            payload.path, self.compute_pool, stage_name="payload_stage", args=["foo", "--delay", "1"]
        )
    ...
    def test_job_decorator(self) -> None:
        @jobs.remote(self.compute_pool, "payload_stage")
    ...
    def test_job_with_spec_overrides(self, *, expected_string: str, **kwargs) -> None:
        # Create a job
