...
class TestRegistryCatBoostModelInteg(registry_model_test_base.RegistryModelTestBase):
    ...
    def test_catboost_classifier_with_params_forwarding(self) -> None:
        """Params declared in the signature reach CatBoostClassifier.predict / predict_proba at runtime."""
        cal_data = datasets.load_breast_cancer(as_frame=True)
        cal_X = cal_data.data
        cal_y = cal_data.target
        cal_X.columns = [inflection.parameterize(c, "_") for c in cal_X.columns]
        ...
        classifier = catboost.CatBoostClassifier(iterations=10, thread_count=1, verbose=False, random_seed=0)
        classifier.fit(cal_X_train, cal_y_train)

        y_pred = classifier.predict(cal_X_test)
        y_pred_proba = classifier.predict_proba(cal_X_test)
        y_pred_few_trees = classifier.predict(cal_X_test, ntree_end=3)
        y_pred_proba_few_trees = classifier.predict_proba(cal_X_test, ntree_end=3)

        param_spec = model_signature.ParamSpec(name="ntree_end", dtype=model_signature.DataType.INT64, default_value=0)
        signatures = {
        ...
        self._test_registry_model(
            ...
            params_assert_fns={
                "predict": (
                    ...
                    lambda res: pd.testing.assert_frame_equal(
                        res,
                        pd.DataFrame(y_pred_few_trees, columns=res.columns),
                        check_dtype=False,
                    ),
                ),
                "predict_proba": (
                    ...
                    lambda res: pd.testing.assert_frame_equal(
                        res,
                        pd.DataFrame(y_pred_proba_few_trees, columns=res.columns),
                        check_dtype=False,
                    ),
                ),
            },
