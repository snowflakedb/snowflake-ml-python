...
class TestRegistryModelingModelInteg(registry_model_test_base.RegistryModelTestBase):
    ...
    def test_snowml_model_deploy_snowml_sklearn_no_explain_by_default(self) -> None:
        iris_X = datasets.load_iris(as_frame=True).frame
        iris_X.columns = [s.replace(" (CM)", "").replace(" ", "") for s in iris_X.columns.str.upper()]
        ...
        # Explainability is opt-in: even with sample_input_data provided, no explain method is generated by default.
        self._test_registry_model(
            model=regr,
            prediction_assert_fns={
                "predict": (
    ...
    def test_snowml_model_deploy_xgboost_no_explain_by_default(self) -> None:
        iris_X = datasets.load_iris(as_frame=True).frame
        iris_X.columns = [s.replace(" (CM)", "").replace(" ", "") for s in iris_X.columns.str.upper()]
    ...
    def test_snowml_model_deploy_lightgbm_no_explain_by_default(self) -> None:
        iris_X = datasets.load_iris(as_frame=True).frame
        iris_X.columns = [s.replace(" (CM)", "").replace(" ", "") for s in iris_X.columns.str.upper()]
