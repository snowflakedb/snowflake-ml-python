...
class TestExecuteInferenceJobServiceHuggingFacePipelineVllmInteg(
    ...
    def test_text_generation_with_vllm(
        ...
        validator = registry_execute_inference_job_service_test_base.create_openai_chat_completion_output_validator(
    ...
    def test_text_generation_with_vllm_and_params(self) -> None:
        ...
        This test uses ParamSpec to pass inference parameters via Input
        instead of including them in the input DataFrame. It validates that
        the model produces output content when parameters are passed via Input.
        """
        model = huggingface.TransformersPipeline(
            model="Qwen/Qwen2.5-0.5B-Instruct",
            task="text-generation",
        ...
        # Parameters are passed via Input.
        x_df = pd.DataFrame.from_records(
            [
                {
                    "messages": [
