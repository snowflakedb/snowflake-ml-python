"""Integration tests for mv.run() with non-ParamSpec OpenAI signatures.
...
Each deployment is reused across mv.run and REST FLAT subtests.

Deployments: 2 signatures × 2 engines × 2 logging styles = 8
"""
...
class TestRegistryMvRun(registry_model_deployment_test_base.RegistryModelDeploymentTestBase):
    """Tests for mv.run() with non-ParamSpec OpenAI signatures."""
    ...
    @parameterized.named_parameters(  # type: ignore[misc]
        *[
            ...
            for log, pool in [("local", None), ("remote", DEFAULT_CPU_COMPUTE_POOL)]
            for sig, fmt in zip(
