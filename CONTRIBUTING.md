...
The project uses Bazel tags for merge gate test selection. The `merge_gate` mode runs affected tests tagged with
`short_regress`.

When writing a new test, add the `short_regress` tag to its `py_test` BUILD target if you want it to run in the merge
gate. Add the tag to tests that are high-signal, fast, and stable. Omit or remove the tag from tests that are slow,
flaky, or testing specialized cases.
...
**Running merge gate tests locally:**

```sh
# Run tests with short_regress tag
bazel test --test_tag_filters=short_regress //...

# Via CI scripts
./ci/RunBazelAction.sh test -m merge_gate
./ci/build_and_run_tests.sh . --mode merge_gate --env pip

# short_regression is an alias for merge_gate
./ci/RunBazelAction.sh test -m short_regression
```
