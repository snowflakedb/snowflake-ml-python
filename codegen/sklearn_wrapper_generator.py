...
class WrapperGeneratorFactory:
    ...
    def get_snow_ml_module_name(module_name: str) -> str:
        ...
        """
        if WrapperGeneratorFactory._is_xgboost(module_name):
            print(f'SnowML module name {"snowflake.ml."+module_name}')
            return "snowflake.ml." + module_name
        else:
            return "snowflake.ml." + ".".join(module_name.split(".")[1:])
    ...
    def read(class_object: Tuple[str, type], module_name: str) -> "WrapperGeneratorBase":
        ...
        """
        generator = (
            XGBoostWrapperGenerator(module_name=module_name, class_object=class_object)
            if WrapperGeneratorFactory._is_xgboost(module_name=module_name)
            else SklearnWrapperGenerator(module_name=module_name, class_object=class_object)
        )
...
class WrapperGeneratorBase:
    ...
    def _format_default_value(self, default_value: Any) -> str:
        ...
        if (isinstance(default_value, int) or isinstance(default_value, float)) and str(default_value) in NP_CONSTANTS:
            import_stmt = "import numpy"
            if import_stmt not in self.estimator_imports_list:
                self.estimator_imports_list.append(import_stmt)
            return f"numpy.{str(default_value)}"
    ...
    def _populate_function_names_and_signatures(self) -> None:
        ...
        for k, v in self.original_init_signature.parameters.items():
            ...
            # Arguments for the call of the underlying sklearn transform.
            sklearn_init_lines.append(f"{v.name}={v.name}")
            init_member_args.append(f"self.{v.name} = self._sklearn_object.{v.name}")
...
class SklearnWrapperGenerator(WrapperGeneratorBase):
    def generate(self) -> "SklearnWrapperGenerator":
        ...
        if self._is_heterogeneous_ensemble:
            ...
            self.test_estimator_input_args_list.append("estimator=SkLogisticRegression(random_state=0)")
        ...
        self.fit_sproc_deps = (
            self.predict_udf_deps
        ) = "'numpy==1.23.4', 'pandas==1.4.4', 'scikit-learn==1.1.3', 'scipy==1.9.3'"
...
class XGBoostWrapperGenerator(WrapperGeneratorBase):
    def generate(self) -> "XGBoostWrapperGenerator":
        ...
        self.fit_sproc_deps = (
            self.predict_udf_deps
        ) = "'numpy==1.23.4', 'pandas==1.4.4', 'scipy==1.9.3', 'xgboost==1.5.0'"
        self._construct_string_from_lists()
        return self
